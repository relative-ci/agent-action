...
async function getCiEnv(config, logger) {
    ...
    if (ciEnv.service === 'jenkins' && !ciEnv.slug) {
        const jenkinsSlug = slug.getSlugFromGitURL(env.JENKINS_GIT_URL);
        logger.debug('Jenkins vars', { slug: jenkinsSlug });
        ciEnv.slug = jenkinsSlug;
    }
    // GitHub extra data
    if (env.GITHUB_EVENT_PATH) {
        const gitHubEnv = await github.getGitHubEnv(env.GITHUB_EVENT_PATH, { includeCommitMessage }, logger);
