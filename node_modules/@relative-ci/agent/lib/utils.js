...
const MASK = '******';
const debug = exports.debug = (0, _debug.default)(DEBUG_NAME);
 ...
 */
function maskObjectProperties(data, propertyPaths) {
  const normalizedData = (0, _merge.default)({}, data);
  propertyPaths.forEach(propertyPath => {
    const value = (0, _get.default)(normalizedData, propertyPath, '');
    (0, _set.default)(normalizedData, propertyPath, maskToken(value));
  });
...
function getCommitMessage() {
  let message = '';
  try {
    message = _child_process.default.execSync('git log -1 --pretty=%B').toString().trim();
  } catch (err) {
...
function getEnvVars() {
  ...
  const envCIvars = (0, _envCi.default)();
